...
// Inline SVG glyphs for social platforms. The source (About Us contributor
// cards) uses an icon font (wknd-icon-font) that is not available in EDS, so
// equivalent SVG marks are rendered here. Link text is preserved as aria-label.
const SOCIAL_ICONS = {
...
export default function decorate(block) {
  ...
  // Decorate each card body: mark the role line and turn the social links into
  // an icon row with inline SVG glyphs.
  ul.querySelectorAll('.cards-team-card-body').forEach((body) => {
    const role = body.querySelector('h3 ~ p, h3 + p');
    if (role) role.classList.add('role');

    const socialList = body.querySelector('ul');
    if (socialList) {
      socialList.classList.add('social-row');
      socialList.querySelectorAll('a').forEach((a) => {
        const key = socialKey(a);
        if (key && SOCIAL_ICONS[key]) {
          a.setAttribute('aria-label', `${a.textContent.trim()} social media`);
