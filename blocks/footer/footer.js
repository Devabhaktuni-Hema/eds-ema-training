...
// Inline SVG glyphs for social platforms. The source uses an icon font
// (wknd-icon-font) which is not available in EDS, so equivalent SVG marks are
// rendered here. The links and labels themselves live in footer.plain.html.
const SOCIAL_ICONS = {
...
export default async function decorate(block) {
  ...
  // Resolve relative nav/footer image paths to their served /content location.
  ...
  const [brandSection, navSection, followSection, legalSection] = sections;
  ...
  if (followSection) {
    follow.append(...followSection.childNodes);
    // Replace social link text with SVG icons (labels preserved as aria-label).
    follow.querySelectorAll('ul a').forEach((a) => {
    ...
    const socialList = follow.querySelector('ul');
    if (socialList) socialList.classList.add('footer-social');
  }

  // --- Top row: brand | nav | follow ---
  const top = document.createElement('div');
  top.className = 'footer-top';
  top.append(brand, navCol, follow);
